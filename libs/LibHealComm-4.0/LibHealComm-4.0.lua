...
HealComm.combatLogHealTimer = HealComm.combatLogHealTimer or {}
...
local spellData, hotData, tempPlayerList, pendingHeals, pendingHots, combatLogHealTimer = HealComm.spellData, HealComm.hotData, HealComm.tempPlayerList, HealComm.pendingHeals, HealComm.pendingHots, HealComm.combatLogHealTimer
...
local function parseDirectHeal(casterGUID, spellID, amount, castTime, ...)
	...
	if unit == "player" then
		...
		if combatLogHealTimer[casterGUID] and GetTime() - combatLogHealTimer[casterGUID] < 0.5 then
			endTime = combatLogHealTimer[casterGUID] + (castTime or 1.5)
		else
...
function HealComm:COMBAT_LOG_EVENT_UNFILTERED(...)
	...
	elseif( eventType == "SPELL_CAST_START" and HealComm:GUIDHasHealed(sourceGUID)) then
		combatLogHealTimer[sourceGUID] = GetTime()
	end
end
